...
LogHandler = handlers.TimedRotatingFileHandler(filename='../cap1/chatlog/chatbot.log', when='midnight',
...
# 전처리 객체 생성
p = Preprocess2(word2index_dic='../cap1/chatbot2_dic.bin', userdic='../cap1/user_dic.tsv')
print("텍스트 전처리가 로드 완료")

intent = IntentModel(model_name='../cap1/intent_model.h5', preprocess=p)
print("의도 파악 모델 로드 완료")

df = pd.read_excel('../cap1/질문 답변 데이터 12주차.xlsx')
print("엑셀 파일 로드 완료")

create_embedding_data = CreateEmbeddingData(df=df, preprocess=p)
create_embedding_data.create_pt_file()
embedding_data = torch.load('../cap1/embedding_data.pt')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if now.weekday() == 1 and query == "질문 답변 데이터 업로드":
            intent = IntentModel(model_name='../cap1/intent_model.h5', preprocess=p)
            print("의도 파악 모델 로드 완료")
            n += 1
            df = pd.read_excel('../cap1/질문 답변 데이터 13주차.xlsx')
            print("엑셀 파일 로드 완료")
            create_embedding_data = CreateEmbeddingData(df=df, preprocess=p)
            create_embedding_data.create_pt_file()
            embedding_data = torch.load('../cap1/embedding_data.pt')
